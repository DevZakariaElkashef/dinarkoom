            ...
            <div class="row">
                <div class="col-md-6 d-flex justify-content-center">
                    <div class="card" style="width: 18rem;">
                        <img style="max-height: 300px; width: 100%" id="rightPreview"
                            src="{{ $rightImage ? Storage::url($rightImage->image) : asset('site/assets/img/header-sides.jpg') }}"
                            class="card-img-top" alt="...">
                        <div class="card-body text-center">
                            <h5 class="card-title text-center">{{ __('Right_Image') }}</h5>

                            <form
                                action="@if ($rightImage) {{ route('banners.update', $rightImage) }} @else {{ route('banners.update', 0) }} @endif"
                                ...
                                @if (auth()->user()->can('edit ads'))
                                    ...
                                    <input type="text" class="form-control mt-2" name="url"
                                        placeholder="{{ __('URL') }}"
                                        value="{{ $rightImage ? $rightImage->url : '' }}">
                                    @error('right_image')
                ...
                <div class="col-md-6 d-flex justify-content-center">
                    <div class="card" style="width: 18rem;">
                        <img style="max-height: 300px; width: 100%" id="leftPreview"
                            src="{{ $leftImage ? Storage::url($leftImage->image) : asset('site/assets/img/header-sides.jpg') }}"
                            class="card-img-top" alt="...">
                        <div class="card-body text-center">
                            <h5 class="card-title text-center">{{ __('Left_Image') }}</h5>

                            <form
                                action="@if ($leftImage) {{ route('banners.update', $leftImage) }} @else {{ route('banners.update', 0) }} @endif"
                                method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                @if (auth()->user()->can('edit ads'))
                                    ...
                                    <input type="text" class="form-control mt-2" name="url"
                                        placeholder="{{ __('URL') }}" value="{{ $leftImage ? $leftImage->url : '' }}">
                                    @error('left_image')
                                        <div class="text-danger">
                                            {{ $message }}
                                        </div>
